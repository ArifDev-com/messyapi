...
async function bootSession(sessionId) {
  ...
  const client = new Client({
    ...
    puppeteer: {
      ...
      args: [
        ...
        '--disable-features=TranslateUI,BlinkFeatures',
  ...
  client.on('ready', () => {
    ...
    // Prevent idle disconnects by pinging the browser state every 30 seconds
    setInterval(async () => {
        try {
            // Evaluates a simple expression in the browser context to maintain the socket
            const state = await client.getState();
            console.log(`Keep-alive ping. Current state: ${state}`);
        } catch (error) {
            console.error('Idle timeout detected or browser lost. Re-initializing...', error);
            try {
                await client.destroy();
            } catch (e) {
                console.error('Error destroying client:', e);
            }
            client.initialize(); // Force restart if it freezes
        }
    }, 30000); // 30 seconds
  });
